...
class Statistics:
    def __init__(self, numerical_list:list):
        self.numerical_list= numerical_list
    
    def count(self):
        return len(self.numerical_list)
    
    def sum_number(self):
        total = 0
        for i in range(len(self.numerical_list)):
            total += self.numerical_list[i]
        return total
    
    def maximum(self):
        high_value = None
        for i in range(len(self.numerical_list)):
            if high_value == None or high_value<= self.numerical_list[i]:
                high_value = self.numerical_list[i]
            else:
                continue
        return high_value
    ...
    def minimum(self):
        low_value = None
        for i in range(len(self.numerical_list)):
            if low_value == None or low_value >= self.numerical_list[i]:
                low_value = self.numerical_list[i]
            else: continue
        return low_value
    def mean(self):
    ...
    def median(self):
        sorted_list = sorted(self.numerical_list)
        n = len(sorted_list)
        if n % 2 == 0:
    ...
    def mode(self):
        ...
        return modes
    def range(self):
        highest = max(self.numerical_list)
        lowest = min(self.numerical_list)
...
if __name__ == "__main__":
   ...
   "Variance": stats.variance(),
    "Standard Deviation": stats.st_deviation(),
   "Frequency Distribution": stats.frequency_distribution()
    }
